...
import { decompressDeflateRaw, decompressZstd } from "@higma-document-io/fig/parser";
...
describe("fig file parsing (generated data)", () => {
  it("validates header", () => {
    ...
    expect(isFigCanvas(file)).toBe(true);

    const header = parseFigCanvasHeader(file);
  ...
  it("rebuilds header correctly", () => {
    ...
    const rebuiltHeader = buildFigCanvasHeader(header.payloadSize, header.version);
  ...
  it("decompresses payload with inflateRaw", () => {
    const { file } = createSampleFigFile();
    const header = parseFigCanvasHeader(file);
    const payload = getFigCanvasPayload(file);
...
describe("fig roundtrip (encode -> build -> parse -> decode)", () => {
  it("preserves data through full roundtrip", () => {
    ...
    expect(isFigCanvas(figFile)).toBe(true);

    // Parse back
    const header = parseFigCanvasHeader(figFile);
    const payload = getFigCanvasPayload(figFile);
    const chunks = splitFigChunks(payload, header.payloadSize);
...
describe("fig file parsing (sample-file.fig)", () => {
  ...
  it("parses canvas.fig from sample-file.fig archive", async () => {
    ...
    expect(isFigCanvas(data)).toBe(true);
    ...
    const payload = getFigCanvasPayload(data);
