...
import { createTranslationMatrix, createRotationMatrix, multiplyMatrices } from "@higma-document-models/fig/matrix";
...
/** Public base methods for concrete fluent shape builders. */
export type FluentShapeBuilder<TCustom> = TCustom & {
  name: (name: string) => FluentShapeBuilder<TCustom>;
  size: (width: number, height: number) => FluentShapeBuilder<TCustom>;
  position: (x: number, y: number) => FluentShapeBuilder<TCustom>;
  rotation: (degrees: number) => FluentShapeBuilder<TCustom>;
  /** Set fill — accepts a Color (solid) or a Paint object (gradient, image, etc.) */
  fill: (colorOrPaint: Color | Paint) => FluentShapeBuilder<TCustom>;
  noFill: () => FluentShapeBuilder<TCustom>;
  stroke: (color: Color) => FluentShapeBuilder<TCustom>;
  strokeWeight: (weight: number) => FluentShapeBuilder<TCustom>;
  strokeCap: (cap: StrokeCap) => FluentShapeBuilder<TCustom>;
  ...
  /** Mark this node as a mask for subsequent siblings */
  mask: (isMask?: boolean) => FluentShapeBuilder<TCustom>;
};
