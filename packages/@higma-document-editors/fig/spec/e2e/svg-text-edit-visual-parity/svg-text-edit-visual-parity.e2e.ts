...
const WRAPPED_TEXT_GUID = WRAPPED_TEXT.guidKey;
...
test.describe("Fig editor SVG text editing visual parity", () => {
  ...
  test("keeps text rendering in the SVG backend while overlay text is not duplicated", async ({ page }) => {
    ...
    const editingState = await page.evaluate(() => {
      const svg = document.querySelector<SVGSVGElement>("[role='group'][aria-label='Fig editor viewport surface'] svg[aria-hidden='true']");
...
async function textInteriorClip(
  ...
  const rect = await nodeScreenRect(page, node);
  return {
    x: rect.left + 6,
    y: rect.top + 6,
