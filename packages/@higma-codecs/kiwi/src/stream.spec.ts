...
describe("StreamingFigDecoder", () => {
  it("yields node changes one at a time", () => {
    const { payload, schemaChunkSize, expectedNodes } = createSampleFigPayload();

    const chunks = splitFigChunks(payload, schemaChunkSize);
  ...
  it("decodes header separately", () => {
    const { payload, schemaChunkSize } = createSampleFigPayload();
...
describe("Fig-family payload roundtrip", () => {
  it("creates and parses raw schema/message chunks", () => {
    const schema = createTestSchema();
    ...
    // Build raw canvas payload chunks.
