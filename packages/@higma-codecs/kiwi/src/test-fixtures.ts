/**
 * @file Kiwi message fixtures for tests without external files.
 */
...
/** Create an enum definition for a fixture schema. */
...
// =============================================================================
// Fig-family payload test data
// =============================================================================

/**
 * Build fig-family raw canvas payload chunks from schema and message data.
 *
 * @param schema - Kiwi schema
 * @param messageData - Uncompressed message data
 * @returns Raw canvas payload and compressed schema chunk size
 */
export function buildTestFigPayload(
  schema: KiwiSchema,
  messageData: Uint8Array,
): { readonly payload: Uint8Array; readonly schemaChunkSize: number } {
  ...
  // Combine all parts
  const result = new Uint8Array(compressedSchema.length + dataChunk.length);
  result.set(compressedSchema, 0);
  result.set(dataChunk, compressedSchema.length);

  return { payload: result, schemaChunkSize: compressedSchema.length };
}

/**
 * Create sample fig-family payload data with sample document structure.
 *
 * @returns Raw canvas payload bytes ready for codec tests
 */
export function createSampleFigPayload(): {
  payload: Uint8Array;
