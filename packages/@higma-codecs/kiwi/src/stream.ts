...
export class StreamingFigDecoder {
  ...
  *decodeNodeChanges(data: Uint8Array): Generator<DecodedNodeChange> {
    ...
    if (!nodeChangesField) {
      throw new KiwiParseError("No nodeChanges field in Message type");
    }
...
export class StreamingFigEncoder {
  ...
  writeNodeChange(node: Record<string, unknown>): void {
    if (this.nodeCountOffset < 0) {
      throw new KiwiParseError("Must call writeHeader before writeNodeChange");
    }
    if (this.finalized) {
      throw new KiwiParseError("Encoder already finalized");
    }
