/**
 * @file Identifier primitives — slug + identifier helpers shared by
 * any package that needs to derive a stable ASCII name from a Figma
 * (or arbitrary Unicode) input.
 *
 * Lives in `@higma-primitives/identifier` so multiple tools — token
 * extraction, JSX emit, fig refinement, etc — share one source of
 * truth for word-splitting, slug formation, and de-duplication.
 *
 ...
 *     React component name. Leading digits get a `T` prefix because
 *     JSX parses lowercase tags as DOM elements.
 *
 * Both go through the same `splitWords` step so the two
