  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\n` +
        `Run: bun packages/@higma-document-renderers/fig/scripts/generate-symbol-resolution-fixtures.ts`,
    );
  }
...
describe("Symbol Resolution", () => {
  beforeAll(async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@higma-document-renderers/fig/scripts/generate-symbol-resolution-fixtures.ts");
      return;
    }
    await loadFigFile();
