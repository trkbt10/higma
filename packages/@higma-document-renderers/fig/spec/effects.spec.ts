...
async function loadFigFile(): Promise<ParsedData> {
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\nRun: bun packages/@higma-document-renderers/fig/scripts/generate-effect-fixtures.ts`,
    );
  }
...
describe("Effect Fixture Debug", () => {
  it("lists frames in effects.fig", async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@higma-document-renderers/fig/scripts/generate-effect-fixtures.ts");
      return;
    }
