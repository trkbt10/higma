/**
 * @file Unit spec for the effect-bounds expansion helpers.
 *
...
describe("computeNodeEffectExpansion", () => {
  ...
  it("DROP_SHADOW with spread adds to every side", () => {
    const exp = computeNodeEffectExpansion(makeNode([
      { type: "DROP_SHADOW", radius: 4, spread: 2, offset: { x: 0, y: 0 }, visible: true },
    ]));
  ...
  it("DROP_SHADOW with positive offset > radius+spread still shows zero expansion on the opposite side", () => {
    ...
    const exp = computeNodeEffectExpansion(makeNode([
      { type: "DROP_SHADOW", radius: 1, offset: { x: 5, y: -5 }, visible: true },
    ]));
    expect(exp).toEqual({ left: 0, right: 6, top: 6, bottom: 0 });
  });

  it("LAYER_BLUR expands all sides uniformly by radius", () => {
    const exp = computeNodeEffectExpansion(makeNode([
  ...
  it("FOREGROUND_BLUR behaves the same as LAYER_BLUR", () => {
    const exp = computeNodeEffectExpansion(makeNode([
      { type: "FOREGROUND_BLUR", radius: 5, visible: true },
    ]));
  ...
  it("INNER_SHADOW contributes zero outward expansion", () => {
    const exp = computeNodeEffectExpansion(makeNode([
      { type: "INNER_SHADOW", radius: 20, offset: { x: 0, y: 0 }, visible: true },
    ]));
  ...
  it("invisible effects contribute nothing", () => {
    const exp = computeNodeEffectExpansion(makeNode([
      { type: "DROP_SHADOW", radius: 12, offset: { x: 0, y: 0 }, visible: false },
    ]));
    expect(exp).toEqual({ left: 0, right: 0, top: 0, bottom: 0 });
  });
  ...
  it("multiple effects pick per-side max, never sum", () => {
    const exp = computeNodeEffectExpansion(makeNode([
      ...
      { type: "DROP_SHADOW", radius: 4, offset: { x: 4, y: 0 }, visible: true },
  ...
  it("accepts Kiwi-enum-shaped effect types", () => {
    // Real .fig parses surface effect.type as `{ value, name }` from Kiwi.
    const exp = computeNodeEffectExpansion(makeNode([
      { type: { value: 1, name: "DROP_SHADOW" } as unknown as "DROP_SHADOW", radius: 2, offset: { x: 0, y: 0 }, visible: true },
    ]));
...
describe("computeRootEffectExpansion", () => {
  ...
  it("takes the per-side maximum across multiple root nodes", () => {
    const exp = computeRootEffectExpansion([
      makeNode([{ type: "DROP_SHADOW", radius: 4, offset: { x: 4, y: 0 }, visible: true }]),
      makeNode([{ type: "DROP_SHADOW", radius: 4, offset: { x: -4, y: 0 }, visible: true }]),
    ]);
