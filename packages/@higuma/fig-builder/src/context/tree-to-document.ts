...
import type { NodeTreeResult, FigBlob, FigGuid } from "@higuma/fig/parser";
import { getNodeType, guidToString, parseGuidString } from "@higuma/fig/parser";
import { walkTree } from "@higuma/fig/tree";
import {
...
} from "@higuma/fig/symbols";
import type { GuidTranslationMap, FigResolveContext } from "@higuma/fig/symbols";
import { defensiveMark } from "@higuma/fig/diagnostics/defensive";
import type { LoadedFigFile } from "@higuma/fig/roundtrip";
import type {
...
import type { BlendMode } from "@higuma/fig/types";
import { DEFAULT_PAGE_BACKGROUND, EMPTY_FIG_STYLE_REGISTRY } from "@higuma/fig/domain";
import { guidToNodeId, guidToPageId } from "@higuma/fig/domain";
 ...
 *     The kiwi-side runtime resolver (`@higuma/fig/symbols/symbol-resolver`)
...
function resolveOverridePaths(
  ...
  const resolve = (
    ...
    // `isInstanceSelfOverride` in `@higuma/fig/symbols`. Centralising
...
function resolveEntryPaths(
  ...
  // `@higuma/fig/symbols/symbol-resolver` and the variant-switch
 ...
 * primitive comes from `@higuma/fig/tree:walkTree` — local DFS
