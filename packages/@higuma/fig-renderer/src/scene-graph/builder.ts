...
import type { FigPaint, FigVectorPath } from "@higuma/fig/types";
import { FIG_NODE_TYPE } from "@higuma/fig/types";
import { guidToString, type FigImage, type FigBlob } from "@higuma/fig/parser";
import { styleRefKeys, reresolveOverridesForVariant } from "@higuma/fig/symbols";
import { dfsById } from "@higuma/fig/tree";
import { IDENTITY_MATRIX } from "@higuma/fig/matrix";
import {
  extractBaseProps,
  extractSizeProps,
  extractPaintProps,
...
import { resolveChildConstraints } from "@higuma/fig/symbols";
 ...
 * `mergeSymbolProperties` in @higuma/fig/symbols, which checks
 ...
 * `@higuma/fig-builder/context`), so Pass 2 finds its targets
...
function applySymbolOverridesToChildren(
  ...
  // is imported from @higuma/fig/symbols — the same SoT that hosts
...
function resolveDesignInstance(
  ...
  // This mirrors `mergeSymbolProperties` in @higuma/fig/symbols (the SoT
  ...
  // Mirrors `resolveInstanceLayout` in @higuma/fig/symbols: the INSTANCE
