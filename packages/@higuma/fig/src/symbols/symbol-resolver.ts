...
import type { FigNode, MutableFigNode, FigKiwiSymbolData, FigKiwiSymbolOverride, FigGuidPath, FigComponentPropAssignment, FigDerivedTextData } from "@higuma/fig/types";
import { guidToString, getNodeType, safeChildren, type FigGuid, type FigBlob } from "@higuma/fig/parser";
import { walkTree } from "@higuma/fig/tree";
import { extractSymbolIDPair } from "@higuma/fig/symbols";
 ...
 * from `symbol-resolver`/`@higuma/fig/symbols` don't break.)
 */
...
function applyComponentPropAssignments(
  ...
  // Walk via the SoT primitive (`@higuma/fig/tree:walkTree`) — the
...
export function resolveInstanceNode(
  ...
  // fix in domain-side
  // resolveOverridePaths (`@higuma/fig-builder:tree-to-document.ts`).
