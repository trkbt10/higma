 ...
 * in `@higuma/fig-builder/context`. Both share the same primitive:
...
/**
 * Structural subset of `FigDesignNode` (defined in `@higuma/fig/domain`)
 * used by the variant re-resolver. `@higuma/fig/symbols` does not
 * import from `@higuma/fig/domain` to keep the dependency direction
