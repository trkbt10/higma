/**
 * @file Argument parsing for the fig-to-godot CLI.
 *
 *   fig-to-godot --input <path-to-.fig> --out <dir>
 *                [--page <canvas-name>]
 *                [--frame <frame-name> | --all]
 *                [--list]
 *
 * Defaults:
 *   --page  defaults to "Design" (matches fig-to-swiftui / fig-to-web)
...
export type CliOptions = {
  ...
  /**
   * Opt-in: hoist StyleBoxFlat sub-resources shared by ≥ 2 emitted
   * scenes into a single `Themes/<themeName>.tres` Theme resource.
   * Default `false` — every scene stays self-contained.
   */
  readonly sharedTheme: boolean;
  /** Theme `.tres` file name (no extension). Default `"Default"`. */
  readonly themeName: string;
};

const USAGE = [
  "fig-to-godot --input <fig-file> --out <dir> [options]",
  "",
  "Options:",
  "  --input <path>          Source .fig file (required)",
  "  --out <dir>             Output directory for generated .tscn files (required unless --list)",
  ...
  "  --shared-theme          Hoist StyleBoxes shared across ≥ 2 scenes into Themes/<name>.tres",
  "  --theme-name <name>     Theme file name (no extension). Default: Default",
...
function stepArgs(argv: readonly string[], i: number, acc: Accumulator): Accumulator {
  ...
  switch (token) {
    ...
    case "--shared-theme":
      return stepArgs(argv, i + 1, { ...acc, sharedTheme: true });
    case "--theme-name":
      return stepArgs(argv, i + 2, { ...acc, themeName: expectValue("--theme-name", argv[i + 1]) });
    default:
...
export function parseArgs(argv: readonly string[]): CliOptions {
  ...
  return {
    ...
    sharedTheme: accumulated.sharedTheme === true,
    themeName: accumulated.themeName ?? "Default",
  };
}
