/**
 * @file Verify proxy bootstrap (no template available in source) lands a
 *  structurally-valid fig file.
 *
 * The contract: when a fixture carries zero FILL or TEXT style proxies,
 * `applyPlan` must still create the proxies the agent named. The
 ...
 * input — bootstrap is supposed to *grow* the file's proxy set, not
 * change anything else's semantics.
 *
 * `rectangle/rectangle.fig` is the chosen subject. Its tree carries
 * SOLID-painted RECTANGLEs with no shared style proxies, so an active
...
describe("refine-fig proxy bootstrap", () => {
  it(
    "creates fill proxies in a file with no template, lint stays clean",
    async () => {
      ...
      // Sanity: the fixture genuinely has zero style proxies before
      ...
      expect(beforeFill, `${FIXTURE} must start with zero FILL proxies`).toBe(0);
      expect(beforeText, `${FIXTURE} must start with zero TEXT proxies`).toBe(0);
      ...
      if (inventory.typography.length > 0) {
        expect(result.textProxiesCreated, "expected at least one bootstrapped TEXT proxy when TEXT is present")
          .toBeGreaterThan(0);
      }
      ...
      const bootstrapSkips = result.skipped.filter(
        (s) =>
          s.action.kind === "create-fill-proxy" ||
          s.action.kind === "create-text-proxy" ||
