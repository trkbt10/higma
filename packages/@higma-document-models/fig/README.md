...
`createFigFile()` ベースの命令型ビルダーは Phase 0b/0c で廃止された。
新 API は **immutable な `FigDesignDocument` を組み立てて `exportFig`
に渡す** 形に統一されている (`@higma-document-io/fig` パッケージ経由)。
...
// ドキュメント + ID カウンタ
const empty = createEmptyFigDesignDocument("My Design");
...
// Internal Only Canvas (Figma インポート必須)
...
// フレームを追加 — fillGeometry は documentToTree が自動合成する
...
// ファイル生成 — meta.json / thumbnail.png / schema 全て自動付与
const exported = await exportFig(frameResult.doc);
const data = exported.data; // Uint8Array
```

### 有効なファイルの要件

`exportFig` が以下を自動的に書き込むので、呼び出し側は意識しなくてよい:

1. **DOCUMENT** ノード（ルート） — `documentToTree` が合成
2. **CANVAS** ノード（ページ、1つ以上） — `createEmptyFigDesignDocument`
   が 1 ページを既定で持つ
3. **Internal Only Canvas** — `addPage({internalOnly: true})` で追加
4. **meta.json**（ファイル名、背景色等） — `exportFresh` が
   `defaultFreshMetadata` を合成
5. **thumbnail.png** — `defaultFreshThumbnail` が 1x1 PNG を合成
6. **strokeWeight / strokeAlign / strokeJoin** — `applyNodeTypeDefaults`
   が各ノードタイプ毎の load-bearing 値を補完
7. **fillGeometry** — `documentToTree` が FRAME / SYMBOL / RECTANGLE /
   ROUNDED_RECTANGLE / ELLIPSE / SECTION + VECTOR に対し
   `encodeRectangleBlob` / `encodeEllipseBlob` /
   `encodeRoundedRectangleBlob` / `encodeSvgPathBlob` を呼んで合成
...
```typescript
import { loadFigFile, saveFigFile } from "@higma-document-models/fig";
...
- [fig-kiwi (npm)](https://www.npmjs.com/package-document-models/fig-kiwi)
- [evanw-codecs/kiwi (GitHub)](https://github.com/evanw-codecs/kiwi)
- [Figma .fig file parser online](https://madebyevan.com/figma-file-parser/)
