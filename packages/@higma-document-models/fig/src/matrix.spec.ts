/** @file FigMatrix operation tests. */

import type { FigMatrix } from "./types";
import { computePreRotationTopLeft, extractRotationDegrees } from "./matrix";

function translationMatrix(x: number, y: number): FigMatrix {
...
describe("computePreRotationTopLeft", () => {
  it("returns the transform translation for unrotated nodes", () => {
    expect(computePreRotationTopLeft(translationMatrix(100, 200), 50, 30)).toEqual({ x: 100, y: 200 });
  });
