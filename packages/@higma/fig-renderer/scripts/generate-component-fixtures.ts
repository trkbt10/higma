 ...
 *
 * Usage:
 *   bun packages/@higma/fig-renderer/scripts/generate-component-fixtures.ts
 */

import * as fs from "node:fs";
import * as path from "node:path";
import { fileURLToPath } from "node:url";
import { createFigFile, frameNode, symbolNode, instanceNode, textNode, roundedRectNode } from "@higma/fig-builder/fig-file";
...
async function generateComponentFixtures(): Promise<void> {
  ...
  console.log(`4. Run: npx vitest run packages/@higma/fig-renderer/spec/components.spec.ts`);
}
