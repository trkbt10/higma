 ...
 *
 * Usage: bun packages/@higma/fig-renderer/scripts/generate-snapshots.ts [scope]
 *
 * Example:
 *   bun packages/@higma/fig-renderer/scripts/generate-snapshots.ts text-comprehensive
 */

import * as fs from 'node:fs';
import * as path from 'node:path';
import { parseFigFile, buildNodeTree, findNodesByType, getNodeType } from "@higma/fig/parser";
import type { FigNode, FigGuid } from "@higma/fig/types";
...
if (!scope) {
  console.log("Usage: bun packages/@higma/fig-renderer/scripts/generate-snapshots.ts [scope]");
  console.log("\nAvailable fixtures:");
  const dirs = fs.readdirSync(FIXTURES_DIR).filter((d) => fs.statSync(path.join(FIXTURES_DIR, d)).isDirectory());
  dirs.forEach((d) => console.log(`  - ${d}`));
  process.exit(1);
