...
import { readPng, writePng, createPngImage } from "@higma/png";
import {
  parseFigFile,
  buildNodeTree,
  findNodesByType,
  ...
  if (!fs.existsSync(FIG_FILE)) {
    throw new Error(
      `Fixture file not found: ${FIG_FILE}\n` +
        `Run: bun packages/@higma/fig-renderer/scripts/generate-constraint-edge-cases-fixtures.ts`,
    );
  }
...
describe("Constraint Edge Cases", () => {
  beforeAll(async () => {
    if (!fs.existsSync(FIG_FILE)) {
      ...
      console.log("Run: bun packages/@higma/fig-renderer/scripts/generate-constraint-edge-cases-fixtures.ts");
      return;
    }
    await loadFigFile();
