/**
 * @file Diamond gradient shader.
 */

export const diamondGradientVertexShader = `
...
export const diamondGradientFragmentShader = `
  ...
  void main() {
    ...
    vec2 delta = abs(localPos - u_center);
    float maxReach = max(max(u_center.x, 1.0 - u_center.x), max(u_center.y, 1.0 - u_center.y));
    float t = clamp((delta.x + delta.y) / max(maxReach, 0.001), 0.0, 1.0);
