...
import type { FigPaint, FigVectorPath } from "@higma/fig/types";
import { FIG_NODE_TYPE } from "@higma/fig/types";
import { guidToString, type FigImage, type FigBlob } from "@higma/fig/parser";
import { styleRefKeys, reresolveOverridesForVariant } from "@higma/fig/symbols";
import { dfsById } from "@higma/fig/tree";
import { IDENTITY_MATRIX } from "@higma/fig/matrix";
import {
  extractBaseProps,
  extractSizeProps,
  extractPaintProps,
...
import { resolveChildConstraints } from "@higma/fig/symbols";
 ...
 * `mergeSymbolProperties` in @higma/fig/symbols, which checks
 ...
 * `@higma/fig-builder/context`), so Pass 2 finds its targets
...
function applySymbolOverridesToChildren(
  ...
  // is imported from @higma/fig/symbols — the same SoT that hosts
...
function resolveDesignInstance(
  ...
  // This mirrors `mergeSymbolProperties` in @higma/fig/symbols (the SoT
  ...
  // Mirrors `resolveInstanceLayout` in @higma/fig/symbols: the INSTANCE
