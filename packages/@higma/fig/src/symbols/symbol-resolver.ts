...
import type { FigNode, MutableFigNode, FigKiwiSymbolData, FigKiwiSymbolOverride, FigGuidPath, FigComponentPropAssignment, FigDerivedTextData } from "@higma/fig/types";
import { guidToString, getNodeType, safeChildren, type FigGuid, type FigBlob } from "@higma/fig/parser";
import { walkTree } from "@higma/fig/tree";
import { extractSymbolIDPair } from "@higma/fig/symbols";
 ...
 * from `symbol-resolver`/`@higma/fig/symbols` don't break.)
 */
...
function applyComponentPropAssignments(
  ...
  // Walk via the SoT primitive (`@higma/fig/tree:walkTree`) — the
...
export function resolveInstanceNode(
  ...
  // fix in domain-side
  // resolveOverridePaths (`@higma/fig-builder:tree-to-document.ts`).
