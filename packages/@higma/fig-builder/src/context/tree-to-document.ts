...
import type { NodeTreeResult, FigBlob, FigGuid } from "@higma/fig/parser";
import { getNodeType, guidToString, parseGuidString } from "@higma/fig/parser";
import { walkTree } from "@higma/fig/tree";
import {
...
import type { GuidTranslationMap, FigResolveContext } from "@higma/fig/symbols";
import { defensiveMark } from "@higma/fig/diagnostics/defensive";
import type { LoadedFigFile } from "@higma/fig/roundtrip";
import type {
...
import type { BlendMode } from "@higma/fig/types";
import { DEFAULT_PAGE_BACKGROUND, EMPTY_FIG_STYLE_REGISTRY } from "@higma/fig/domain";
import { guidToNodeId, guidToPageId } from "@higma/fig/domain";
 ...
 *     The kiwi-side runtime resolver (`@higma/fig/symbols/symbol-resolver`)
...
function resolveOverridePaths(
  ...
  const resolve = (
    ...
    // `isInstanceSelfOverride` in `@higma/fig/symbols`. Centralising
...
function resolveEntryPaths(
  ...
  // `@higma/fig/symbols/symbol-resolver` and the variant-switch
 ...
 * primitive comes from `@higma/fig/tree:walkTree` — local DFS
