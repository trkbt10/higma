...
export class ByteBuffer {
  ...
  seek(offset: number): void {
    if (offset < 0 || offset > this._length) {
      throw new KiwiParseError(`Seek out of bounds: ${offset}`, this._offset);
    }
    this._offset = offset;
  }
  ...
  readByte(): number {
    if (this._offset >= this._length) {
      throw new KiwiParseError("Unexpected end of buffer", this._offset);
    }
  ...
  readVarUint(): number {
    ...
    while (true) {
      ...
      shift += 7;
      if (shift > 35) {
        throw new KiwiParseError("VarUint too large", this._offset);
      }
    }
  }
  ...
  readVarUint64(): bigint {
    ...
    while (true) {
      ...
      shift += 7n;
      if (shift > 63n) {
        throw new KiwiParseError("VarUint64 too large", this._offset);
      }
    }
  }
  ...
  readFloat32(): number {
    if (this._offset >= this._length) {
      throw new KiwiParseError("Cannot read Float32", this._offset);
    }
  ...
  readUint32LE(): number {
    if (this._offset + 4 > this._length) {
      throw new KiwiParseError("Cannot read Uint32", this._offset);
    }
