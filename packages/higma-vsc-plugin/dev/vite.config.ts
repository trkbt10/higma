/**
 * @file Vite config for the higma-vsc-plugin browser playground.
 *
 * Hosts the same `webview/index.tsx` the VS Code extension loads inside
 * its custom editor, but with `acquireVsCodeApi` stubbed by `main.tsx`,
 * VS Code-themed CSS variables provided by `index.html`, and a dev-only
 * toolbar for switching fixtures or dropping in arbitrary `.fig` files.
 *
 * Workspace `@higma-*` aliases mirror `spec/e2e/vite.config.ts` so we
 * exercise the live source tree, not built output. Keep the two alias
 * lists in sync — there is no shared module because the divergence has
 * historically been zero and the duplication is small.
 */
